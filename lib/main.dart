...
class VeriDeposu {
  ...
  static void programiKaydet(List<List<Map<String, dynamic>>> t, String tur) {
    ...
    programArsivi.add(KayitliProgramGecmisi(
        tarih: DateTime.now(), tur: tur, programVerisi: yeniProgramListesi));
    puanEkle("101", 100);
  }
  ...
  static void rozetleriGuncelle() {
    int d = denemeListesi.length;
    int k = tamamlananKonular.values.where((e) => e).length;
    int p = ogrenciler[0].puan;
    for (var r in tumRozetler) {
      if (r.kategori == "Deneme") r.mevcutSayi = d;
      if (r.kategori == "Konu") r.mevcutSayi = k;
      if (r.kategori == "Seviye") r.mevcutSayi = p;
  ...
  static String yeniKayit(
    ...
    if (ogrenciler.any((o) => o.tcNo == tc) ||
        ogretmenler.any((t) => t.tcNo == tc)) return "Bu TC zaten kayıtlı!";
    if (rol == "Öğrenci") {
      ogrenciler.add(Ogrenci(
          id: tc.substring(8),
          tcNo: tc,
...
// MODELLER
class Ogrenci {
  String id, tcNo, sifre, ad, sinif, fotoUrl;
  int puan, girisSayisi;
...
final Map<String, List<KonuDetay>> dersKonuAgirliklari = {
  ...
  "TYT Türkçe": [
    ...
    KonuDetay("İsim-Sıfat-Zamir", 2),
    KonuDetay("Fiil-Zarf-Edat", 2),
    KonuDetay("Cümlenin Ögeleri", 1),
    KonuDetay("Cümle Türleri", 1),
    KonuDetay("Anlatım Bozukluğu", 1)
  ],
  "TYT Matematik": [
    KonuDetay("Temel Kavramlar", 3),
    KonuDetay("Sayı Basamakları", 2),
    KonuDetay("Bölme - Bölünebilme", 2),
    KonuDetay("EBOB - EKOK", 2),
    KonuDetay("Rasyonel Sayılar", 2),
    ...
    KonuDetay("Oran - Orantı", 2),
    KonuDetay("Sayı Problemleri", 5),
    KonuDetay("Kesir Problemleri", 2),
    KonuDetay("Yaş Problemleri", 1),
    KonuDetay("İşçi Emek Problemleri", 1),
    KonuDetay("Hız Problemleri", 1),
    ...
    KonuDetay("İkinci Dereceden Denklemler", 1),
    KonuDetay("Permütasyon Kombinasyon", 2),
    KonuDetay("Olasılık", 1),
    KonuDetay("Veri - İstatistik", 1),
    KonuDetay("Mantık", 1)
  ],
  "TYT Tarih": [
    KonuDetay("Tarih Bilimine Giriş", 1),
    KonuDetay("Uygarlığın Doğuşu ve İlk Uygarlıklar", 1),
    ...
    KonuDetay("Beylikten Devlete", 1),
    KonuDetay("Dünya Gücü Osmanlı", 1),
    KonuDetay("Osmanlı Kültür ve Medeniyeti", 1),
    KonuDetay("Yeni Çağ Avrupası", 1),
    KonuDetay("Arayış Yılları", 1),
    KonuDetay("18. Yüzyılda Değişim ve Diplomasi", 1),
    KonuDetay("En Uzun Yüzyıl", 1),
    KonuDetay("20. Yüzyıl Başlarında Osmanlı", 1),
    KonuDetay("Kurtuluş Savaşı Hazırlık", 2),
    KonuDetay("Kurtuluş Savaşı Cepheler", 2),
    KonuDetay("İlke ve İnkılaplar", 2),
  ...
  "TYT Kimya": [
    ...
    KonuDetay("Atom ve Periyodik Sistem", 2),
  ...
  "TYT Biyoloji": [
    ...
    KonuDetay("Hücre ve Organelleri", 1),
  ...
  "AYT Edebiyat": [
    ...
    KonuDetay("Servet-i Fünun ve Fecr-i Ati", 2),
    KonuDetay("Milli Edebiyat", 2),
    KonuDetay("Cumhuriyet Dönemi (Şiir)", 2),
    KonuDetay("Cumhuriyet Dönemi (Roman)", 2),
    KonuDetay("Cumhuriyet Dönemi (Tiyatro/Öğretici)", 2),
    KonuDetay("Edebi Akımlar", 1)
  ],
  "AYT Matematik": [
    KonuDetay("Fonksiyonlar-2", 2),
    KonuDetay("Polinomlar", 2),
    KonuDetay("İkinci Dereceden Denklemler", 2),
    KonuDetay("Parabol", 1),
    KonuDetay("Eşitsizlikler", 2),
    KonuDetay("Trigonometri", 5),
    KonuDetay("Logaritma", 3),
    KonuDetay("Diziler", 2),
    KonuDetay("Limit ve Süreklilik", 3),
    KonuDetay("Türev", 6),
    KonuDetay("İntegral", 6)
  ],
  "AYT Tarih": [
    KonuDetay("Tarih Bilimi", 1),
    KonuDetay("İlk Uygarlıklar", 1),
    KonuDetay("İlk Türk Devletleri", 1),
    KonuDetay("İslam Tarihi", 1),
    KonuDetay("Türk İslam Devletleri", 2),
    KonuDetay("Osmanlı Devleti (Kuruluş)", 1),
    KonuDetay("Osmanlı Devleti (Yükselme)", 2),
    KonuDetay("Osmanlı Devleti (Kültür Medeniyet)", 2),
    KonuDetay("Milli Mücadele", 4),
    KonuDetay("Atatürkçülük ve İnkılaplar", 2)
  ],
  "AYT Coğrafya": [
    KonuDetay("Ekosistemler", 1),
    KonuDetay("Nüfus Politikaları", 1),
    KonuDetay("Yerleşme Tipleri", 1),
    KonuDetay("Ekonomik Faaliyetler", 1),
    KonuDetay("Türkiye Ekonomisi", 2),
    KonuDetay("Bölgeler ve Ülkeler", 2),
    KonuDetay("Çevre ve Toplum", 1)
  ],
  "AYT Felsefe": [
    KonuDetay("Psikoloji", 3),
    KonuDetay("Sosyoloji", 3),
    KonuDetay("Mantık", 3)
  ],
  "AYT Din Kültürü": [
    KonuDetay("Kuran ve Yorumu", 1),
    KonuDetay("Hz. Muhammed", 1),
    KonuDetay("İslam Düşüncesinde Yorumlar", 1),
    KonuDetay("Din ve Hayat", 1)
  ],
  "AYT Fizik": [
    KonuDetay("Vektörler", 1),
    KonuDetay("Bağıl Hareket", 1),
    KonuDetay("Newton Hareket Yasaları", 1),
  ...
  "AYT Kimya": [
    KonuDetay("Modern Atom Teorisi", 2),
    KonuDetay("Gazlar", 1),
    KonuDetay("Sıvı Çözeltiler", 2),
...
void _initMufredat() {
  if (tumDerslerGlobal.isEmpty) {
    dersKonuAgirliklari.forEach((key, value) {
      ...
      int soruSayisi = key.contains("Matematik")
          ? 40
          : key.contains("Fizik")
              ? 14
              : 10;
                    ...
                    Hero(
                        ...
                        child: Container(
                            ...
                            child: const Icon(Icons.school_rounded,
                                size: 64, color: AppTheme.primaryDark))),
...
class _KisiselBilgiEkraniState extends State<KisiselBilgiEkrani> {
  late TextEditingController a, s, n;
  String f = "";
...
class OgrenciAnaEkrani extends StatelessWidget {
  final String ogrenciId;
  final bool readOnly;
  const OgrenciAnaEkrani(
      {super.key, required this.ogrenciId, this.readOnly = false});
  ...
  void _cikisYap(BuildContext context) {
    Navigator.pushAndRemoveUntil(context,
        MaterialPageRoute(builder: (c) => const GirisEkrani()), (r) => false);
  }
            ...
            sliver: SliverGrid.count(
                ...
                children: [
                  _menuBtn(context, Icons.check_box, "KONU TAKİP", Colors.blue,
                      KonuTakipEkrani(readOnly: readOnly)),
                  _menuBtn(context, Icons.table_view, "PROGRAMIM",
                      Colors.blueGrey, const TumProgramEkrani()),
                  _menuBtn(
                      context,
                      Icons.view_list,
                      "DENEME LİSTESİ",
                  ...
                  if (!readOnly) ...[
                    _menuBtn(context, Icons.check_circle, "GÜNLÜK TAKİP",
                        Colors.teal, const GunlukTakipEkrani()),
                    ...
                    _menuBtn(context, Icons.school, "OKUL NOTLARI",
                        Colors.brown, const OkulSinavlariEkrani()),
...
// ============================================================
// SORU TAKİP EKRANI (GÜNCELLENDİ)
// ============================================================
class SoruTakipEkrani extends StatefulWidget {
  final String ogrenciId;
  const SoruTakipEkrani({super.key, required this.ogrenciId});
  @override
  State<SoruTakipEkrani> createState() => _SoruTakipEkraniState();
}

class _SoruTakipEkraniState extends State<SoruTakipEkrani> {
  String? secilenDers;
  String? secilenKonu;
  final _dogruC = TextEditingController();
  final _yanlisC = TextEditingController();

  void _kaydet() {
    if (secilenDers == null ||
        secilenKonu == null ||
        _dogruC.text.isEmpty ||
        _yanlisC.text.isEmpty) {
    ...
    VeriDeposu.soruEkle(SoruCozumKaydi(
        ...
        ders: secilenDers!,
        konu: secilenKonu!,
        dogru: int.tryParse(_dogruC.text) ?? 0,
        yanlis: int.tryParse(_yanlisC.text) ?? 0,
        tarih: DateTime.now()));
    setState(() {
      _dogruC.clear();
      _yanlisC.clear();
  ...
  @override
  Widget build(BuildContext context) {
    var liste = VeriDeposu.soruCozumListesi
    ...
    return Scaffold(
        appBar: AppBar(
            title: const Text("Soru Takip ve İstatistik"),
                      ...
                      child: Column(children: [
                        ...
                        if (secilenDers != null)
                          DropdownButton<String>(
                              ...
                              value: secilenKonu,
                              items: dersKonuAgirliklari[secilenDers]!
                        ...
                        return Card(
                            child: ListTile(
                                leading: CircleAvatar(
                                    ...
                                    child: Text("${k.dogru + k.yanlis}")),
                                title: Text(k.ders),
                                subtitle: Text(
                                    "${k.konu} (${k.tarih.day}/${k.tarih.month})"),
                                trailing: Text("D:${k.dogru} Y:${k.yanlis}",
                                    style: const TextStyle(
                                        fontWeight: FontWeight.bold))));
                      }))
            ])));
...
class _KonuTakipEkraniState extends State<KonuTakipEkrani> {
  double _oran(String d) {
    ...
    for (var k in l) {
      t += k.agirlik;
      if (VeriDeposu.tamamlananKonular["$d - ${k.ad}"] == true) c += k.agirlik;
    }
    return t == 0 ? 0 : c / t;
  }
...
class _DenemeEkleEkraniState extends State<DenemeEkleEkrani> {
  ...
  List<DersGiris> tytMat = [DersGiris("Matematik", 40)];
        ...
        child: DefaultTabController(
            ...
            child: Scaffold(
                ...
                body: TabBarView(children: [
                  _form("TYT", tytTurkce + tytSosyal + tytMat + tytFen),
                  _form("AYT", aytMat + aytFen + aytEa + aytSoz)
                ]))));
  }
      ...
      Expanded(
          child: ListView(
              children: l
                  ...
                  .toList())),
      ElevatedButton(onPressed: () => _kaydet(t), child: const Text("KAYDET"))
    ]);
  }
}
...
class _YapayZekaAsistanEkraniState extends State<YapayZekaAsistanEkrani>
  ...
  void initState() {
    super.initState();
    _konuListesiGuncelle();
  }

  void _konuListesiGuncelle() {
    bool sadeceTYT = (secilenSinif == "9" || secilenSinif == "10");
    konuDurumlari.clear();
    dersKonuAgirliklari.forEach((ders, konular) {
      bool ekle = true;
      if (sadeceTYT && ders.contains("AYT")) ekle = false;
      if (secilenAlan == "Sayısal" &&
          (ders.contains("Edebiyat") ||
              ders.contains("Sosyal") ||
              ders.contains("Tarih") ||
              ders.contains("Coğrafya"))) ekle = false;
      if (secilenAlan == "Sözel" &&
          (ders.contains("Matematik") ||
              ders.contains("Fen") ||
              ders.contains("Fizik") ||
              ders.contains("Kimya") ||
              ders.contains("Biyoloji"))) ekle = false;
      if (ekle) {
        for (var k in konular) konuDurumlari["$ders - ${k.ad}"] = true;
      }
    });
  }
  ...
  void _yapayZekaProgramOlustur() async {
    ...
    konuDurumlari.forEach((key, value) {
      if (value) {
        var parts = key.split(" - ");
        havuz.add({'ders': parts[0], 'konu': parts[1]});
      }
    });
    ...
    showDialog(
        ...
        builder: (context) => AlertDialog(
                ...
                content: DropdownButton<String>(
                    isExpanded: true,
                    value: dersKonuAgirliklari.containsKey(yeniDers)
                        ? yeniDers
                        : null,
                    items: dersKonuAgirliklari.keys
                        .map((e) => DropdownMenuItem(value: e, child: Text(e)))
  ...
  void _kaydetVeCik() {
    VeriDeposu.programiKaydet(akilliProgramTablosu, "Yapay Zeka");
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              ...
              return Padding(
                  ...
                  child: Row(children: [
                    ElevatedButton(
                        ...
                        child: Text(_currentStep == 2 ? "HAZIRLA" : "İLERLE")),
                    const SizedBox(width: 10),
                    if (_currentStep > 0)
                      TextButton(
            ...
            steps: [
              Step(
                  ...
                  content: Column(children: [
                    ...
                    if (secilenSinif == "11" || secilenSinif == "12")
                      DropdownButtonFormField(
                          value: secilenAlan,
...
class _ManuelProgramEkraniState extends State<ManuelProgramEkrani>
    ...
    dersKonuAgirliklari.forEach((k, v) {
      ...
      if (alan == "Sayısal" &&
          (k.contains("Edebiyat") ||
              k.contains("Sosyal") ||
              k.contains("Tarih") ||
              k.contains("Coğrafya"))) ekle = false;
      if (alan == "Sözel" &&
          (k.contains("Matematik") ||
              k.contains("Fen") ||
              k.contains("Fizik") ||
              k.contains("Kimya") ||
              k.contains("Biyoloji"))) ekle = false;
  ...
  void _programOlustur() {
    tb.clear();
    int toplamDk = (bitisSaati.hour * 60 + bitisSaati.minute) -
        (baslangicSaati.hour * 60 + baslangicSaati.minute);
    int blokSuresi = 60;
          ...
          for (int k = 0; k < dersSayisi; k++) {
            int dersBaslamaDk =
                (baslangicSaati.hour * 60 + baslangicSaati.minute) +
                    (k * blokSuresi);
            String saat =
                "${(dersBaslamaDk ~/ 60).toString().padLeft(2, '0')}:${(dersBaslamaDk % 60).toString().padLeft(2, '0')}";
    ...
    showDialog(
        ...
        builder: (context) => AlertDialog(
                ...
                content: DropdownButton<String>(
                    ...
                    onChanged: (v) {
                      setState(() {
                        tb[haftaIndex][gunIndex]['bloklar'][blokIndex]['ders'] =
                            v;
                        tb[haftaIndex][gunIndex]['bloklar'][blokIndex]['konu'] =
                            "Manuel Seçim";
                      });
                      Navigator.pop(context);
                    }),
  ...
  Widget build(BuildContext context) {
    if (_hazir)
      return Scaffold(
          appBar: AppBar(
              title: const Text("Taslak Program"),
            ...
            Expanded(
                child: TabBarView(
                    ...
                    children: tb
                        ...
                        .entries
                        .map((entry) =>
                            _buildHaftaTablosu(entry.key, entry.value))
                        .toList())),
            const Divider(),
            const Text("KAYITLI GEÇMİŞ PROGRAMLAR",
                style:
                    TextStyle(fontWeight: FontWeight.bold, color: Colors.grey)),
            Expanded(
                child: ListView.builder(
                    itemCount: VeriDeposu.programArsivi.length,
                    itemBuilder: (c, i) {
                      var kayit = VeriDeposu.programArsivi[i];
                      return ListTile(
                          title: Text(kayit.tur),
                          subtitle: Text(
                              "${kayit.tarih.day}.${kayit.tarih.month} - ${kayit.tarih.hour}:${kayit.tarih.minute}"),
                          trailing:
                              Row(mainAxisSize: MainAxisSize.min, children: [
                            TextButton(
                                child: const Text("UYGULA"),
                                onPressed: () {
                                  VeriDeposu.arsivdenProgramiYukle(kayit);
                                  ScaffoldMessenger.of(context).showSnackBar(
                                      const SnackBar(
                                          content: Text("Program yüklendi!")));
                                }),
                            IconButton(
                                icon:
                                    const Icon(Icons.delete, color: Colors.red),
                                onPressed: () {
                                  setState(() => VeriDeposu.arsivSil(kayit));
                                })
                          ]));
                    }))
          ]),
          floatingActionButton:
            ...
            FloatingActionButton.extended(
                heroTag: "btn2",
                onPressed: () {
                  VeriDeposu.programiKaydet(tb, "Manuel - ${DateTime.now()}");
                  Navigator.pop(context);
                  showDialog(
                      context: context,
                      builder: (c) => const AlertDialog(
                          title: Text("Başarılı"),
                          content: Text(
                              "Program Kaydedildi ve PDF İndirildi (Simülasyon)")));
                },
                label: const Text("KAYDET & PDF"),
                icon: const Icon(Icons.picture_as_pdf))
          ]));

    return Scaffold(
        appBar: AppBar(
            title: const Text("Program Sihirbazı"),
            backgroundColor: Colors.orange),
        body: Stepper(
            currentStep: _currentStep,
            onStepContinue: () {
            ...
            steps: [
                      ...
                      DropdownButtonFormField(
                          ...
                          items: ["Sayısal", "Sözel", "Eşit Ağırlık", "Dil"]
                              .map((s) =>
                                  DropdownMenuItem(value: s, child: Text(s)))
                              .toList(),
                          onChanged: (v) => setState(() => alan = v!)),
                    Slider(
                        value: sure,
                        min: 1,
                        max: 24,
                        divisions: 23,
                        label: "${sure.toInt()} Hafta",
              ...
              Step(
                  ...
                  content: Column(children: [
                    DropdownButtonFormField(
                        value: calismaStili,
                        decoration:
                            const InputDecoration(labelText: "Çalışma Stili"),
                        items: ["25+5 Dk (Pomodoro)", "40+5 Dk", "50+10 Dk"]
                            .map((s) =>
                                DropdownMenuItem(value: s, child: Text(s)))
                            .toList(),
                        onChanged: (v) => setState(() => calismaStili = v!)),
                    Row(children: [
                      TextButton(
                          onPressed: () async {
                            var t = await showTimePicker(
              ...
              Step(
                  ...
                  content: Container(
                      height: 300,
                      child: ListView(
                          children: secilenDersler.keys
                              .map((k) => CheckboxListTile(
                                  title: Text(k),
                                  value: secilenDersler[k],
                                  onChanged: (v) =>
                                      setState(() => secilenDersler[k] = v!)))
                              .toList()))),
            ]));
  }
  ...
  Widget build(BuildContext context) {
    if (len == 0)
      return Scaffold(
          ...
          body: const Center(child: Text("Henüz kayıtlı program yok.")));
    return Scaffold(
        appBar: AppBar(
            title: const Text("Tüm Program"),
  ...
  Widget build(BuildContext context) {
    var l = ogrenciId != null
        ? VeriDeposu.denemeListesi
            ...
            .toList()
        : VeriDeposu.denemeListesi;
    var k = VeriDeposu.kurumsalDenemeler;
    return DefaultTabController(
        ...
        child: Scaffold(
            appBar: AppBar(
                ...
                bottom: const TabBar(
                    tabs: [Tab(text: "Sonuçlar"), Tab(text: "Kurumsal PDF")])),
            body: TabBarView(children: [
              ListView.builder(
                  itemCount: l.length,
                  itemBuilder: (c, i) => Card(
                      child: ListTile(
                          ...
                          trailing:
                              Text("${l[i].tarih.day}/${l[i].tarih.month}")))),
              k.isEmpty
                  ? const Center(child: Text("Kurumsal Deneme Yok"))
                  : ListView.builder(
                      itemCount: k.length,
                      itemBuilder: (c, i) => Card(
                          child: ListTile(
                              leading: const Icon(Icons.picture_as_pdf,
                                  color: Colors.red),
                              title: Text(k[i].baslik),
                              subtitle: const Text("Açmak için tıkla"),
                              onTap: () {
                                showDialog(
                                    context: context,
                                    builder: (c) => AlertDialog(
...
class _YoneticiPaneliState extends State<YoneticiPaneli> {
  ...
  void _pdfEkle() {
    ...
    showDialog(
        context: context,
        builder: (c) => AlertDialog(
                title: const Text("PDF Deneme Ekle"),
                content: TextField(
                    controller: t,
                ...
                actions: [
                  ElevatedButton(
                      onPressed: () {
                        VeriDeposu.pdfDenemeEkle(t.text);
                        Navigator.pop(c);
                        ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text("Yayınlandı!")));
                      },
  ...
  @override
  Widget build(BuildContext context) {
    var ist = VeriDeposu.getIstatistikler();
    List<Widget> pages = [
      Padding(
          padding: const EdgeInsets.all(20),
          child: Column(children: [
            _stat("Öğrenci Sayısı", ist["Öğrenci Sayısı"]!, Colors.blue),
            _stat("Öğretmen Sayısı", ist["Öğretmen Sayısı"]!, Colors.teal),
            _stat("Toplam Deneme", ist["Toplam Deneme"]!, Colors.orange),
            _stat("Sistem Girişleri", ist["Sistem Girişleri"]!, Colors.purple)
          ])),
      DefaultTabController(
          ...
          child: Column(children: [
            const TabBar(
                ...
                tabs: [Tab(text: "Öğrenciler"), Tab(text: "Öğretmenler")]),
            Expanded(
                child: TabBarView(children: [
              _userList(VeriDeposu.ogrenciler, true),
              _userList(VeriDeposu.ogretmenler, false)
            ]))
          ])),
      Column(children: [
        const SizedBox(height: 20),
        ElevatedButton.icon(
            ...
            label: const Text("PDF DENEME YÜKLE"),
            style: ElevatedButton.styleFrom(
                backgroundColor: Colors.redAccent,
                foregroundColor: Colors.white,
                padding: const EdgeInsets.all(15))),
        Expanded(
            child: ListView.builder(
                itemCount: VeriDeposu.kurumsalDenemeler.length,
                itemBuilder: (c, i) => Card(
                    child: ListTile(
                        leading:
                            const Icon(Icons.picture_as_pdf, color: Colors.red),
                        title: Text(VeriDeposu.kurumsalDenemeler[i].baslik),
                        subtitle: Text(
                            "${VeriDeposu.kurumsalDenemeler[i].tarih.day}/${VeriDeposu.kurumsalDenemeler[i].tarih.month}")))))
      ])
    ];
    return Scaffold(
        appBar: AppBar(
        ...
        bottomNavigationBar: BottomNavigationBar(
            ...
            items: const [
              BottomNavigationBarItem(
                  icon: Icon(Icons.dashboard), label: "Genel"),
              BottomNavigationBarItem(
                  icon: Icon(Icons.people), label: "Kullanıcılar"),
              BottomNavigationBarItem(
                  icon: Icon(Icons.assignment), label: "Denemeler")
            ]));
  }
        ...
        child: Container(
            ...
            child: Row(
                ...
                children: [
                  ...
                  Icon(Icons.info, size: 50, color: Colors.white30)
                ])));
  }

  Widget _userList(List items, bool isStudent) {
    return ListView.builder(
        ...
        itemBuilder: (c, i) {
          ...
          return Card(
              child: ListTile(
                  leading: CircleAvatar(child: Text(u.ad[0])),
                  title: Text(u.ad),
                  subtitle: Text("TC: ${u.tcNo}"),
                    ...
                    if (isStudent)
                      IconButton(
                          ...
                          onPressed: () => Navigator.push(
                              c,
                              MaterialPageRoute(
                                  builder: (ctx) => OgrenciAnaEkrani(
                                      ogrenciId: u.id, readOnly: true)))),
                    IconButton(
                        icon: const Icon(Icons.delete, color: Colors.red),
                        onPressed: () {
                          setState(
                              () => VeriDeposu.kullaniciSil(u.id, isStudent));
                          ScaffoldMessenger.of(context).showSnackBar(
...
class OgretmenPaneli extends StatelessWidget {
  ...
  void _mesaj(BuildContext c, String oid) {
    TextEditingController t = TextEditingController();
    showDialog(
        context: c,
        builder: (ctx) => AlertDialog(
                ...
                actions: [
                  ElevatedButton(
                      onPressed: () {
                        VeriDeposu.mesajGonder("Öğretmen", oid, t.text);
                        Navigator.pop(ctx);
                      },
    ...
    return Scaffold(
        ...
        body: ListView.builder(
            ...
            itemBuilder: (c, i) {
              ...
              return Card(
                  child: ExpansionTile(
                      leading: CircleAvatar(child: Text(o.ad[0])),
                      title: Text(o.ad),
                    ...
                    ListTile(
                        ...
                        onTap: () => _mesaj(context, o.id)),
                    ListTile(
                        leading: const Icon(Icons.visibility),
                        title: const Text("Öğrenciyi İncele"),
                        onTap: () => Navigator.push(
                            context,
                            MaterialPageRoute(
                                builder: (c) => OgrenciAnaEkrani(
                                    ogrenciId: o.id, readOnly: true))))
                  ]));
            }));
  }
}
        ...
        body: GridView.count(crossAxisCount: 2, children: [
          ModernDashboardCard(
              ...
              onTap: () => Navigator.push(
                  ...
                  MaterialPageRoute(
                      builder: (c) =>
                          DenemeListesiEkrani(ogrenciId: ogrenci.id)))),
          ModernDashboardCard(
              icon: Icons.check,
              title: "Konular",
              color: Colors.blue,
              onTap: () => Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (c) => KonuTakipEkrani(readOnly: true)))),
          ModernDashboardCard(
              ...
              onTap: () => Navigator.push(
                  ...
                  MaterialPageRoute(
                      builder: (c) => OgrenciAnaEkrani(
                          ogrenciId: ogrenci.id, readOnly: true))))
        ]));
  }
}
